...
public class TurretBehaviour : MonoBehaviour
{
    ...
    void Shoot()
    {
        if(gameObject.GetComponentsInChildren<ParticleSystem>().Length == 1)
        {
            gameObject.GetComponentInChildren<ParticleSystem>().Play();
        }
        else if(gameObject.GetComponentsInChildren<ParticleSystem>().Length > 1)
        {
            transform.GetChild(Random.Range(0, transform.childCount)).GetComponent<ParticleSystem>().Play();
        }
