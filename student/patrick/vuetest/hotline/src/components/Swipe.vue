<template>
    <div class="SwipePageContainer">
        ...
        <div v-if="BioVisible" class="SwipeImageSelector">
        </div>
        <div @click="ShowBio()" v-html="CurrentImage" class="SwipeImage">

        </div>
        <div v-if="BioVisible" v-html="SinglePerson.Bio" class="SwipeBio">
        </div>
        <div class="SwipeButtons">
            <button @click="LikeAndDislike(this)">Like</button>
            <button @click="LikeAndDislike(this)">Dislike</button>
        </div>
...
export default
{
  data()
  {
    return { // need help with this error.
      ListOfSingles:
      [
      ...
      BioVisible: false,
      SinglePerson: this.ListOfSingles[0],
      ImageButtons: null,
      CurrentImage: `<img src="${SinglePerson.ProfilePictures[0]}" />`,
    }
  },
  ...
  methods:
  {
    RandomSinglePerson()
    {
      return this.ListOfSingles[RandomNumber(0, 5)];
    },
    ...
    ShowBio()
    {
      if (!this.BioVisible)
      {
        let Cache = '';
          ...
          {
              Cache += `<button onclick="ChangeImage('${this.SinglePerson.ProfilePictures[i]}')" ></button>`;
          }
    ...
    LikeAndDislike(Button)
    {
      ...
      this.CurrentImage = `<img src="${SinglePerson.ProfilePictures[0]}" />`;
      this.BioVisible = false;
      if (Button.innerText == "Like" && Math.random() >= 0.5)
      {
...
.SwipePageContainer
{
    ...
    grid-template-areas: 'EmptyLeft SwipeName EmptyRight' 'EmptyLeft SwipeImageSelector EmptyRight' 'EmptyLeft SwipeImage EmptyRight' 'EmptyLeft SwipeBio EmptyRight' 'EmptyLeft SwipeButtons EmptyRight';
}
